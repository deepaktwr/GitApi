...
public class RetrofitClient<T> {
    ...
    public synchronized static RetrofitClient createClient(){
        if(retrofitClient != null) return retrofitClient;
        return retrofitClient = new RetrofitClient<>();
    }

    public T getRetrofitService(Class<T> classObj, int classId){
    ...
    public <V> V getErrorRespose(Class<V> classObj, Response<GitResponse> response) throws IOException {
